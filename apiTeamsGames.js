class ApiTeam {
    constructor(teamId, leagueName, charterType, distanceClauseApplies, initialRating) {
        this.teamId = teamId;
...
const apiTeams = { "17404a": new ApiTeam("17404a","D.H.R. Men's Roller Derby", "primary", false, 515.5) };
...
async function buildTeamsAndGames(callback) {

    groupedApiGames.clear();
    ...
    let unvalidated3Games = config.include_status3_games ? await fetchGames(unvalidatedMinDt, null, 3) : [];
    ...
    if (config.include_game_history) {
